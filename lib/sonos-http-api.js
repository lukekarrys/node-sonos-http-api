...
function HttpAPI(discovery, settings) {
  ...
  function buildWebhookData(type, data) {
    var typeName = 'type'
    var dataName = 'data'
  ...
  this.requestHandler = function (req, res) {
    ...
    if (opt.action === 'events') {
      events.addClient(
        res,
        player.uuid,
        buildWebhookData('transport-state', player)
      )
      return
    }
    ...
    logger.info(id, opt.action, opt.values.join(' '))
    ...
    Promise.resolve(handleAction(opt))
      .then((response) => {
        ...
        logger.info(id, `${Date.now() - start}ms`)
  ...
  function invokeWebhook(type, data) {
    const jsonBody = buildWebhookData(type, data)

    events.sendEvent(jsonBody, data.uuid)
