...
const PlayerPrototypes = [
  require('../prototypes/Player/getUriType.js'),
  require('../prototypes/Player/nightMode.js'),
  require('../prototypes/Player/recalculateGroupVolume.js'),
  require('../prototypes/Player/replaceWithFavorite.js'),
  require('../prototypes/Player/replaceWithPlaylist.js'),
  require('../prototypes/Player/setGroupVolume.js'),
  require('../prototypes/Player/speechEnhancement.js'),
  require('../prototypes/Player/subCrossover.js'),
  require('../prototypes/Player/subDisable.js'),
  require('../prototypes/Player/subEnable.js'),
  require('../prototypes/Player/subGain.js'),
  require('../prototypes/Player/subPolarity.js'),
]
...
function getPlayMode(state) {
  let key =
    (state.shuffle << 1) |
    (state.repeat === REPEAT_MODE.ONE
      ? 4
      : state.repeat === REPEAT_MODE.ALL
      ? 1
      : 0)
...
function deepFreeze(obj) {
  ...
  Object.keys(copy)
    .filter(
      (key) =>
        copy[key] && Object.getPrototypeOf(copy[key]) === Object.prototype
    )
...
function parseTrackMetadata(metadata, nextTrack) {
  return new Promise((resolve, reject) => {
    let track = nextTrack
      ? clone(EMPTY_STATE.nextTrack)
      : clone(EMPTY_STATE.currentTrack)
    ...
    sax.on('tag:item', (item) => {
      ...
      track.duration = item.res
        ? parseTime((item.res.$attrs || item.res).duration)
        : 0
...
function Player(data, listener, system) {
  ...
  Object.defineProperty(_this, 'state', {
    get: () =>
      getState(
        state,
        _this.coordinator._state,
        _this.hasSub ? _this.sub : null
      ),
  })
  ...
  function getPositionInfo() {
    return soap
      ...
      .then((response) => {
        ...
        if (response.reltime !== undefined)
          state.relTime = parseTime(response.reltime)
  ...
  function notificationHandler(uuid, data) {
    ...
    if (data.transportstate) {
      ...
      state.playMode.repeat =
        currentPlayMode & PLAY_MODE.REPEAT_ALL
          ? REPEAT_MODE.ALL
          : currentPlayMode & PLAY_MODE.REPEAT_ONE
          ? REPEAT_MODE.ONE
          : REPEAT_MODE.NONE
      state.playMode.shuffle = !!(
        PLAY_MODE[data.currentplaymode.val] & PLAY_MODE.SHUFFLE_NOREPEAT
      )
      ...
      parseTrackMetadata(data.currenttrackmetadata)
        ...
        .then(() => {
          return parseEnqueuedMetadata(
            data['r:enqueuedtransporturimetadata']
          ).then((enqueuedInfo) => {
...
Player.prototype.removeTrackRangeFromQueue = function removeTrackRangeFromQueue(
  startIndex,
  numberOfTracks
) {
  return soap.invoke(
...
Player.prototype.reorderTracksInQueue = function reorderTracksInQueue(
  startIndex,
  numberOfTracks,
  insertBefore
) {
...
Player.prototype.addURIToQueue = function addURIToQueue(
  uri,
  metadata,
  enqueueAsNext,
...
Player.prototype.addMultipleURIsToQueue = function addMultipleURIsToQueue(
  elements,
  containerURI,
  containerMetadata,
  enqueueAsNext,
...
Player.prototype.setPlayMode = function setPlayMode(newPlayMode) {
  ...
  if (newPlayMode.repeat !== undefined || newPlayMode.shuffle !== undefined) {
    ...
    logger.debug(
      `repeat: ${newPlayMode.repeat}, shuffle: ${newPlayMode.shuffle}`
    )
    ...
    promise = promise.then(() => {
      return soap
        ...
        .catch((err) => {
          logger.warn(
            err,
            `Failed when trying to set playmode ${playMode}, could be playing radiostation or line-in, no worries.`
          )
        })
    })
  }
...
Player.prototype.repeat = function repeat(mode) {
  ...
  if (typeof mode === 'boolean')
    repeatMode = mode ? REPEAT_MODE.ALL : REPEAT_MODE.NONE
...
Player.prototype.becomeCoordinatorOfStandaloneGroup =
  function becomeCoordinatorOfStandaloneGroup() {
    return soap.invoke(
      `${this.baseUrl}/MediaRenderer/AVTransport/Control`,
      ...
      return new Promise((resolve, reject) => {
        ...
        sax.on('tag:item', (item) => {
          returnResult.items.push({
            ...
            albumArtUri:
              item['upnp:albumarturi'] instanceof Array
...
Player.prototype.browseAll = function browseAll(objectId) {
  ...
  let getChunk = (chunk) => {
    ...
    return this.browse(
      objectId,
      chunk.startIndex + chunk.numberReturned,
      0
    ).then(getChunk)
  }
...
Player.prototype.getQueue = function getQueue(limit, offset) {
  ...
  return this.browse(queueIdentifier, offset, limit).then(
    (result) => result.items
  )
}
...
PlayerPrototypes.forEach((proto) => {
