...
function replaceWithFavorite(favoriteName) {
  ...
  return this.system
    ...
    .then((favorites) => {
      logger.debug(
        `found favorites`,
        favorites.map((x) => x.title)
      )
      return favorites.find(
        (fav) =>
          fav.title.toLowerCase() === favoriteName.toLowerCase() ||
          fav.uri.toLowerCase() === favoriteName.toLowerCase()
      )
    })
    .then((favorite) => {
      if (!favorite) {
      ...
      return this.clearQueue()
        .then(() =>
          logger.debug(
            `Adding ${favorite.uri} to queue with metadata ${favorite.metadata}`
          )
        )
    ...
    .then((favorite) => {
      logger.debug(
        `setting AVTransport to ${favorite.uri} with metadata ${favorite.metadata}`
      )
