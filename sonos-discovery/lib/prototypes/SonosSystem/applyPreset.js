...
function setVolume(system, playerPresets) {
  ...
  return playerPresets.reduce((promise, playerInfo) => {
    ...
    return promise
      .then(() => {
        if (Object.prototype.hasOwnProperty.call(playerInfo, 'volume')) {
          logger.debug(
      ...
      .then(() => {
        if (Object.prototype.hasOwnProperty.call(playerInfo, 'mute')) {
          logger.debug(
  ...
  return system.zones
    ...
    .reduce((promise, zone) => {
      return promise.then(() => {
        ...
        return zone.coordinator.pause().catch(() => {})
      })
    }, initialPromise)
}
...
function groupWithCoordinator(players) {
  ...
  return players.slice(1).reduce((promise, player) => {
    if (player.avTransportUri === groupingUri) {
      logger.debug(
        `skipping grouping for ${player.roomName} because it is already grouped with coordinator`
      )
      return promise
    }

    logger.debug(
      `adding ${player.roomName} to coordinator ${coordinator.roomName}`
    )
...
function ungroupFromCoordinator(system, players) {
  ...
  return superfluousPlayers.reduce((promise, player) => {
    logger.debug(
      `ungrouping ${player.roomName} from coordinator ${coordinator.roomName}`
    )
...
function applyPreset(preset) {
  ...
  promise = promise.then(() => {
    players = preset.players.map((playerInfo) =>
      this.getPlayer(playerInfo.roomName)
    )
  })
  ...
  promise = promise.then(() => {
    ...
    if (coordinator.coordinator.uuid !== coordinator.uuid) {
      logger.debug(
        `breaking out ${coordinator.roomName} because player is part of group`
      )
      return coordinator
    ...
    if (players.length === 1 && coordinator.avTransportUri !== preset.uri) {
      logger.debug(
        `breaking out ${coordinator.roomName} because player is coordinator of group and uri differs`
      )
      return coordinator
        .becomeCoordinatorOfStandaloneGroup()
        .then(() => groupWithCoordinator(players))
  ...
  if (preset.favorite) {
    promise = promise.then(() =>
      coordinator.replaceWithFavorite(preset.favorite)
    )
  } else if (preset.playlist) {
    promise = promise.then(() =>
      coordinator.replaceWithPlaylist(preset.playlist)
    )
  } else if (preset.uri) {
    promise = promise.then(() =>
      coordinator.setAVTransport(preset.uri, preset.metadata)
    )
  }
