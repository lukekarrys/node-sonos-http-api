...
const SonosPrototypes = [
  require('./prototypes/SonosSystem/applyPreset.js'),
  require('./prototypes/SonosSystem/getFavorites.js'),
  require('./prototypes/SonosSystem/getPlaylists.js'),
  require('./prototypes/SonosSystem/refreshShareIndex.js'),
]
...
function SonosSystem(settings) {
  ...
  function topologyChange(uuid, topology) {
    ...
    soap
      .invoke(
        `${anyPlayer.baseUrl}/MusicServices/Control`,
        soap.TYPE.ListAvailableServices
      )
      .then(soap.parse)
      .then(parseServices)
...
SonosSystem.prototype.getPlayer = function getPlayer(name) {
  return this.players.find(
    (player) => player.roomName.toLowerCase() === name.toLowerCase()
  )
}
